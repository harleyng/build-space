...
const DIRECTIONS = [
  { value: "Bắc", angle: 0, label: "Bắc" },
  { value: "Đông Bắc", angle: 45, label: "Đông Bắc" },
  { value: "Đông", angle: 90, label: "Đông" },
  { value: "Đông Nam", angle: 135, label: "Đông Nam" },
  { value: "Nam", angle: 180, label: "Nam" },
  { value: "Tây Nam", angle: 225, label: "Tây Nam" },
  { value: "Tây", angle: 270, label: "Tây" },
  { value: "Tây Bắc", angle: 315, label: "Tây Bắc" },
];

export const DirectionCompass = ({ value, onChange, label }: DirectionCompassProps) => {
    ...
    <div className="space-y-4">
          ...
          {DIRECTIONS.map((direction, index) => {
            ...
            return (
              <g key={direction.value}>
                <path
                  d={getSegmentPath(startAngle, endAngle, 80, 180)}
                  className={cn(
      ...
      {value && (
        <p className="text-center text-sm text-muted-foreground">
          Hướng đã chọn: <span className="font-semibold text-foreground">{value}</span>
        </p>
      )}
    </div>
  );
